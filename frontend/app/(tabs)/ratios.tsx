          ...
          return (
            <View key={key} style={styles.ratioCard}>
              ...
              <View style={styles.track}>
                {mine !== null && <View style={[styles.fill, { width: `${minePct}%` }]} />}
                <View style={[styles.needle, { left: `${natPct}%` }]} />
              </View>
              <View style={styles.ratioRow}>
                <Text style={styles.ratioVal}>{mine !== null ? `${mine.toFixed(1)}${cfg.unit}` : 'N/D'}</Text>
                <Text style={styles.ratioNat}>Nationale : <Text style={{ color: Colors.bronze }}>{cfg.nat}{cfg.unit}</Text></Text>
              </View>
            </View>
          );
        })}
...
function SimpleBarChart({ data }: { data: any[] }) {
  ...
  const lastVal = (data ?? []).reduce((s: number, i: any) => s + (i.total_ht || 0), 0) || 2850;
  const vals = [2800, 3100, 2950, 3400, 3200, lastVal];
  const maxV = Math.max(...vals);
  return (
    <View style={styles.chart}>
      {vals.map((v, i) => (
        <View key={i} style={styles.barWrap}>
          <View style={styles.barTrack}><View style={[styles.bar, { height: `${(v / maxV) * 100}%` }]} /></View>
